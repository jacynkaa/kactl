 ...
 * License: ???
 * Source: ????
 * Description: Deque-like undoing on data structures with
...
struct DequeUndo {
	...
	vector<T> elems[2];
	vector<pii> his = {{0,0}};
	// Push object to front or back of deque, depending on side arg.
	void push(T val, bool side) {
		elems[side].pb(val);
		doPush(0, side);
	}
	// Pop object from front or back of deque, depending on side arg.
	void pop(int side) {
	...
	void doPush(int i, bool s) {
		apply([&](auto... x) { ds.insert(x...); },elems[s].rbegin()[i]);
		his.pb({ds.time(), s});
	}
};