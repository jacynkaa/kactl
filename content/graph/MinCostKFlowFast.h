...
struct MCMF {
	...
	MCMF(int N) : g(N), dst(N, 0), pre(N), vis(N) {
	}
	...
	// dij = 0 for spfa, dij = 1 for dijkstra
	void spfa(int s, int dij) {
		fill(all(pre), -1);
		fill(all(vis), 0);
		fill(all(dst), INF);
		dst[s] = 0;
	...
	// computes min cost k-flow
	// set k to -1 for unbounded flow
	pair<ll, ll> calc(int s, int t, ll k) {
