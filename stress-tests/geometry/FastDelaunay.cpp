...
int main1() {
	...
	fwd(it,0,3000000) {{
		...
		if (false) rep(i,N) rep(j,i) rep(k,j) {
			// colinear
			if (ps[i].cross(ps[j], ps[k]) == 0) {  goto fail; }
		}
		if (false) rep(i,N) rep(j,i) rep(k,j) rep(l,k) {
			// concyclic
		...
		if (N >= 3) {
			fwd(i,2,N) if ((ps[i] - ps[0]).cross(ps[1] - ps[0])) allColinear = false;
		}
