...
void testMatching() {
	fwd(it,0,100000) {
		...
		rep(i,N) rep(j,M) co[i][j] = (rand() % 10) + 2;
		vi L, R;
		ll v = MinCostMatching(co, L, R);
		int S = N+M, T = N+M+1;
		MCMF mcmf(N+M+2);
		rep(i,N) mcmf.addEdge(S, i, 1, 0);
		rep(i,M) mcmf.addEdge(N+i, T, 1, 0);
		rep(i,N) rep(j,M) mcmf.addEdge(i, N+j, 1, co[i][j] - 2);
		mcmf.setpi(S);
		auto pa = mcmf.maxflow(S, T);
